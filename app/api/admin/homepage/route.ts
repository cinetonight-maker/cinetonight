...
/* ============================================================================
 * /api/admin/homepage — the Homepage Manager's back end.
 *
...
const NOT_SET_UP = {
  error: "The Homepage Manager needs its database table. Run supabase/homepage_cms.sql in Supabase → SQL Editor, then reload. Nothing on the site is affected until you do.",
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: `Could not load the homepage settings: ${(e as Error).message}` }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    if (action === "saveDraft") {
      ...
      const { error } = await admin.from("homepage_config")
    ...
    if (action === "rollback") {
      ...
      const { data: rev, error: e1 } = await admin
        .from("homepage_revisions").select("*").eq("id", revisionId).maybeSingle();
      if (e1) throw e1;
    ...
    if (action === "publish") {
      ...
      // Snapshot what is being replaced FIRST, so the current homepage is
      ...
      const { error } = await admin.from("homepage_config").update({
        live_config: draft,
        draft_config: null,
        draft_saved_at: null,
      ...
      const revalidated = await revalidateForAction({ kind: "homepage" });
      await recordAudit({
    ...
    if (MISSING((e as Error).message)) return NextResponse.json(NOT_SET_UP, { status: 400 });
    return NextResponse.json({ error: `Could not save the homepage settings: ${(e as Error).message}` }, { status: 500 });
  }
}
